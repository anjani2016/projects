...
# ==========================================
# UIPATH MAESTRO BPMN 2.0 PROCESS SIMULATOR
# ==========================================
import time
from typing import Dict, Any, List, Optional
from pydantic import BaseModel

class BPMNHumanActionRequest(BaseModel):
    decision: str  # "Approve" or "Reject"
    comments: str

# In-memory database of active and completed processes
processes_db = {}

@app.get("/api/uipath/bpmn/providers")
async def get_repair_providers():
    """Retrieve available welding technicians/repair providers."""
    from src.core.use_cases.repair_planner import RepairPlanner
    planner = RepairPlanner()
    return {"status": "success", "providers": planner._load_providers()}

@app.get("/api/uipath/bpmn/processes")
async def get_processes():
    """Retrieve all simulated BPMN processes."""
    return {"status": "success", "processes": list(processes_db.values())}

@app.post("/api/uipath/bpmn/processes")
async def start_process(
    ...
    other_standard: str = Form("None"),
    usage: str = Form("Fabrication"),
    process_name: str = Form("Weld_Inspection_Workflow")
):
    """Starts a new weld inspection process representing a BPMN Start Event."""
    process_id = f"BPMN-PROC-{int(time.time())}"
    report_id = f"REP-BPMN-{int(time.time())}"
    
    file_bytes = await file.read()
    image_hash = hashlib.sha256(file_bytes).hexdigest()
    
    raw_storage_path = f"data/raw/{process_id}.jpg"
    with open(raw_storage_path, "wb") as buffer:
        buffer.write(file_bytes)
    ...
    processes_db[process_id] = {
        ...
        "status": "Running",
        "current_task": "AI Inspection",
        "variables": {
            "report_id": report_id,
            "raw_image_path": raw_storage_path,
            "thickness": thickness,
        ...
        "task_id": None,
        "repair_plan": None,
        "selected_welder": None,
        "defects": [],
        "agent_reasoning": "",
        "history": [
            {
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "event": f"BPMN Process '{process_name}' initiated (Start Event). Radiograph film ingested.",
                "actor": "RPA Robot"
            }
        ]
    }
...
@app.post("/api/uipath/bpmn/processes/{process_id}/step")
async def step_process(process_id: str):
    """Executes the active task or gateway in the BPMN process flow."""
    if process_id not in processes_db:
        raise HTTPException(status_code=404, detail="BPMN process instance not found.")
    ...
    if current_task == "AI Inspection":
        # 1. Run the AI Pipeline (similar to /inspect endpoint)
        raw_storage_path = vars["raw_image_path"]
        annotated_storage_path = f"data/inspections/annotated/{report_id}.jpg"
        ...
        vision_adapter = UltralyticsAdapter(vars["model_path"], db_adapter)
        compliance_adapter = LocalComplianceAdapter(db_adapter)
        defects = vision_adapter.detect(enhanced_img, image_hash=vars["image_hash"])
        process["defects"] = [d.model_dump() for d in defects]
        
        # Run orchestrator for detailed reasoning
        orchestrator = InspectionOrchestrator(vision_adapter, db_adapter, compliance_adapter)
        orchestrator.report_id = report_id
        orchestrator.raw_image_path = f"raw/{report_id}.jpg"
        orchestrator.annotated_image_path = f"annotated/{report_id}.jpg"
        ...
        agent_output = await orchestrator.run(
            ...
            vars["model_path"],
            vars["thickness"],
            image_hash=vars["image_hash"],
            app_type=vars["app_type"],
            material=vars["material"],
            regulatory_code=vars["regulatory_code"],
            client_spec=vars["client_spec"],
            other_standard=vars["other_standard"],
            usage=vars["usage"]
        )
        process["agent_reasoning"] = agent_output
        
        # Determine pass/fail based on reasoning or defects
        is_passed = "STATUS: PASS" in agent_output
        ...
        # Defect Gateway decision routing
        if is_passed:
            process["current_task"] = "Exporting Report"
            process["status"] = "Awaiting Export"
            process["history"].append({
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "event": "AI Inspection Service Task Completed. Weld complied with standards. Routing to report generation.",
                "actor": "Coding Agent"
            })
        else:
            process["current_task"] = "Human Review"
            process["status"] = "Awaiting Review"
            process["task_id"] = f"TSK-REVIEW-{int(time.time())}"
            process["history"].append({
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "event": f"AI Inspection Service Task Completed. B31.3 compliance failed. Escalating User Task: '{process['task_id']}' to Level III Inspector.",
                "actor": "Coding Agent"
            })
            
    elif current_task == "Repair Planning":
        # Run Agentic Repair Planning
        from src.core.use_cases.repair_planner import RepairPlanner
        planner = RepairPlanner()
        plan_results = await planner.generate_plan(
            material=vars["material"],
            regulatory_code=vars["regulatory_code"],
        ...
        process["selected_welder"] = {
            "id": plan_results["selected_welder_id"],
            "name": plan_results["selected_welder_name"],
            "reasoning": plan_results["selection_reasoning"]
        }
        process["repair_plan"] = plan_results["action_plan"]
        process["current_task"] = "Welder Rework"
        process["status"] = "In Rework"
        process["history"].append({
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            "event": f"Agentic Repair Planning Service Task Completed. Matched technician '{plan_results['selected_welder_name']}'. Detailed rework instructions generated.",
            "actor": "Repair Agent"
        })
        
    elif current_task == "Exporting Report":
        # 1. Save record to database
        is_passed = "STATUS: PASS" in process["agent_reasoning"]
        weld_verdict = "PASS" if is_passed else "REJECT"
        
        from src.core.domain.entities import InspectionRecord
        record = InspectionRecord(
            report_id=report_id,
            image_id=vars["raw_image_path"],
            thickness=vars["thickness"],
            model_used=vars["model_path"],
            verdict=weld_verdict,
            details=process["agent_reasoning"],
            raw_image_path=f"raw/{process_id}.jpg",
            annotated_image_path=f"annotated/{report_id}.jpg",
            performer_comments="",
            supervisor_comments="Simulated BPMN flow closure.",
            status_state=2,
            material=vars["material"],
            regulatory_code=vars["regulatory_code"],
            client_spec=vars["client_spec"],
        ...
        try:
            ...
            findings = []
            for idx, d in enumerate(process["defects"]):
                findings.append({
                    "type": str(d.get("type", "Defect")),
                    "size_mm": d.get("dims", {}).get("length", 0.0) * 0.1,
                    "status": "Reject"
                })
            ...
            report_data = {
                ...
                "usage": vars["usage"],
                "findings": findings,
                "agent_reasoning": process["agent_reasoning"],
                "performer_comments": "",
                "supervisor_comments": "BPMN Process closed automatically.",
                "status_state": 2
            }
            ...
            logging.error(f"Failed to create report in BPMN step: {pdf_err}")
            
        # Complete Process
        process["current_task"] = "End Event"
        process["status"] = "Completed"
        process["history"].append({
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            "event": "Report Generation Service Task Completed. Inspection certificate published. BPMN Process Terminated (End Event).",
            "actor": "RPA Robot"
        })
        
    return process

@app.post("/api/uipath/bpmn/processes/{process_id}/action")
async def action_process(process_id: str, request: BPMNHumanActionRequest):
    """Simulates a human inspector signing off on a pending User Task."""
    if process_id not in processes_db:
        raise HTTPException(status_code=404, detail="BPMN process instance not found.")
        
    process = processes_db[process_id]
    if process["current_task"] != "Human Review":
        raise HTTPException(status_code=400, detail="Process is not currently awaiting human review.")
    ...
    if decision == "Approve":
        process["current_task"] = "Exporting Report"
        process["status"] = "Awaiting Export"
        # Force reasoning update to indicate manual approval override
        process["agent_reasoning"] += f"\n\n[MANUAL APPROVAL OVERRIDE BY INSPECTOR]\nComments: {comments}"
        process["history"].append({
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            "event": f"Human Review User Task Resolved. Inspector approved weld with deviation. Comments: {comments}",
            "actor": "Level III NDT Inspector"
        })
    else:
        process["current_task"] = "Repair Planning"
        process["status"] = "Awaiting Repair Plan"
        process["history"].append({
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            "event": f"Human Review User Task Resolved. Inspector rejected weld and ordered repair. Comments: {comments}. Routing to Repair Planning Agent.",
            "actor": "Level III NDT Inspector"
        })
...
@app.post("/api/uipath/bpmn/processes/{process_id}/rework")
async def rework_process(
    process_id: str,
    file: UploadFile = File(...),
    comments: str = Form(...)
):
    """Simulates the Welder rework loop uploading a repaired radiograph."""
    if process_id not in processes_db:
        raise HTTPException(status_code=404, detail="BPMN process instance not found.")
        
    process = processes_db[process_id]
    if process["current_task"] != "Welder Rework":
        raise HTTPException(status_code=400, detail="Process is not currently awaiting welder rework.")
        
    file_bytes = await file.read()
    image_hash = hashlib.sha256(file_bytes).hexdigest()
    
    # Overwrite the raw image with the repair film
    raw_storage_path = process["variables"]["raw_image_path"]
    with open(raw_storage_path, "wb") as buffer:
        buffer.write(file_bytes)
        
    process["variables"]["image_hash"] = image_hash
    process["current_task"] = "AI Inspection"
    process["status"] = "Running"
    process["defects"] = []
    process["repair_plan"] = None
    process["selected_welder"] = None
    
    process["history"].append({
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "event": f"Welder Rework User Task Completed. Repaired weld film uploaded. Comments: {comments}. Looping back to AI Inspection.",
        "actor": "Welding Technician"
    })
    
    return process


@app.post("/api/uipath/repair-planner")
async def uipath_repair_planner(request: Request):
    """Invokes the agentic repair planner directly supporting both JSON and Form payloads."""
    import json
    from src.core.use_cases.repair_planner import RepairPlanner
    ...
    if "application/json" in content_type:
        try:
            ...
            regulatory_code = body.get("regulatory_code", "")
            thickness = float(body.get("thickness", 0.0))
            defects_list = body.get("defects", [])
        except Exception as json_err:
            logging.error(f"Failed to parse JSON body: {json_err}")
    else:
        try:
            form = await request.form()
            material = form.get("material", "")
            regulatory_code = form.get("regulatory_code", "")
            thickness = float(form.get("thickness", 0.0))
            defects_raw = form.get("defects", "[]")
            defects_list = json.loads(defects_raw)
        except Exception as form_err:
            logging.error(f"Failed to parse Form body: {form_err}")
    ...
    return {
        "status": "success",
        "selected_welder_id": plan_results["selected_welder_id"],
        "selected_welder_name": plan_results["selected_welder_name"],
        "selection_reasoning": plan_results["selection_reasoning"],
        "action_plan": plan_results["action_plan"]
    }
