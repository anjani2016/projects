...
## 🤝 Band of Agents (Band.ai) Integration

For the **Band of Agents Hackathon**, the system is expanded to run a fully distributed, multi-agent quality control workflow inside a Band room.
...
*   **Multi-Agent Room Sequence**: 
    1.  **Weld Orchestrator Agent**: Manages the room state, logs auditable events.
    2.  **Weld Vision Agent**: Uses YOLO / RT-DETR to localize physical defects.
    3.  **Weld Compliance Agent**: Verifies dimensions against ASME rules.
    4.  **Weld Review Agent**: Enforces critical safety override thresholds.

For detailed sequence diagrams and flowcharts, see [band_architecture.md](band_architecture.md).
...
This project utilizes radiography weld defect datasets for training and validation:
- **Gazprom Weld Defect Radiographic Dataset**: Public radiography data used for fine-tuning our RT-DETR and YOLOv11 defect detection pipelines.
