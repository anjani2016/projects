...
# Band (Thenvoi) multi-agent orchestrator
from src.band.orchestrator_agent import BandInspectionOrchestrator
...
app = FastAPI(
    title="AI Weld Inspector Backend — Band of Agents",
    version="2.0.0",
    description="Multi-agent NDT inspection system powered by Band (Thenvoi) platform.",
)
...
@app.post("/inspect/band")
async def inspect_weld_band(
    file: UploadFile = File(...),
    thickness: float = Form(...),
    model_path: str = Form(...),
    x_user_role: str = Header("Inspector"),
    ...
    """
    Band multi-agent inspection endpoint.
    Routes the inspection through 4 Band-connected agents:
      1. VisionAgent  — RT-DETR / YOLO defect detection
      2. ComplianceAgent — ASME / AWS / API rule evaluation
      3. OrchestratorAgent — coordinates agents, synthesizes verdict
      4. ReviewAgent — HITL safety sign-off + audit trail

    All agents communicate through the Band (Thenvoi) platform.
    """
    mcp_url = os.environ.get("MONGODB_URI", "")
    db_adapter = MongoAdapter(mcp_url)

    # SHA-256 image hash for vision cache
    file.file.seek(0)
    file_bytes = file.file.read()
    file.file.seek(0)
    image_hash = hashlib.sha256(file_bytes).hexdigest()
    ...
    try:
        # CLAHE image enhancement (same pipeline as /inspect)
        processor    = WeldProcessor()
        enhanced_img = processor.enhance_image(raw_storage_path)
        cv2.imwrite(raw_storage_path, enhanced_img)

        # Run Band multi-agent orchestration
        band_orchestrator = BandInspectionOrchestrator()
        band_result = await band_orchestrator.run_band_inspection(
            image_path=raw_storage_path,
            image_hash=image_hash,
            thickness=thickness,
        ...
        # Build annotated image (green = PASS, red = REJECT)
        defects     = band_result.get("defects", [])
        is_passed   = band_result.get("final_verdict") == "PASS"
        box_color   = (0, 255, 0) if is_passed else (0, 0, 255)
        annotated_img = cv2.cvtColor(enhanced_img, cv2.COLOR_GRAY2BGR)
        for d in defects:
            x1, y1, x2, y2 = map(int, d["bbox"])
        ...
        try:
            ...
            for d in defects:
                ...
                findings.append({
                    "type": d["type"],
                    "size_mm": mm_len,
                    "status": "Reject" if band_result.get("final_verdict") == "REJECT" else "Accept",
                })
            report_data = {
                "report_id": report_id,
                "thickness": thickness,
                ...
                "findings": findings,
                "agent_reasoning": (
                    f"[Band Multi-Agent Result]\n"
                    f"Final Verdict: {band_result.get('final_verdict')}\n"
                    f"Agents: {', '.join(band_result.get('agents_involved', []))}\n"
                    f"Compliance: {band_result.get('compliance_reasoning')}\n"
                    f"Review Notes: {band_result.get('review_notes')}\n"
                    f"Override Applied: {band_result.get('override_applied')}"
                ),
                "performer_comments": "",
                "supervisor_comments": "",
                "status_state": 0,
            }
            WeldReporter().create_report(pdf_path, report_data, annotated_storage_path)
        except Exception as pdf_err:
            logging.error("Band PDF generation failed: %s", pdf_err)
        ...
        return {
            "status": "success",
            "mode": "band_multi_agent",
            "report_id": report_id,
            "result": f"STATUS: {band_result.get('final_verdict')}\n\n{band_result.get('compliance_reasoning')}\n\nReview: {band_result.get('review_notes')}",
        ...
        logging.error("Band inspection endpoint error: %s", exc)
        if os.path.exists(raw_storage_path):
            os.remove(raw_storage_path)
        return {"status": "error", "mode": "band_multi_agent", "result": str(exc)}
