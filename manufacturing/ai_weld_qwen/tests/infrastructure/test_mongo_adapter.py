...
def test_mongo_adapter_dynamic_db_name_resolution(temp_db_path, monkeypatch):
    # Test 1: URI contains database name
    uri_with_db = "mongodb://user:pass@localhost:27017/my_custom_ndt_db?authSource=admin"
    adapter1 = MongoAdapter(connection_string=uri_with_db, sqlite_path=temp_db_path)
    assert adapter1.db_name == "my_custom_ndt_db"
    
    # Test 2: URI with srv scheme
    srv_uri = "mongodb+srv://user:pass@weld.cluster.mongodb.net/srv_ndt_db?retryWrites=true"
    adapter2 = MongoAdapter(connection_string=srv_uri, sqlite_path=temp_db_path)
    assert adapter2.db_name == "srv_ndt_db"
    
    # Test 3: Override via environment variable MONGODB_DB_NAME
    monkeypatch.setenv("MONGODB_DB_NAME", "override_db_name")
    adapter3 = MongoAdapter(connection_string=srv_uri, sqlite_path=temp_db_path)
    assert adapter3.db_name == "override_db_name"
