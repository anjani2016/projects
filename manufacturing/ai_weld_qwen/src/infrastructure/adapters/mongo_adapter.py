...
class MongoAdapter(DatabasePort):
    def __init__(self, connection_string: str, sqlite_path: str = "data/local_ndt.db"):
        ...
        # Resolve database name from URI or fallback to env or default
        self.db_name = "ndt_inspections"
        if self.connection_string and not self.connection_string.startswith("mcp:"):
            try:
                from pymongo.uri_parser import parse_uri
                parsed = parse_uri(self.connection_string)
                if parsed.get("database"):
                    self.db_name = parsed["database"]
            except Exception as e:
                # String-based parsing fallback to support offline test environments (bypassing PyMongo srv DNS lookup)
                try:
                    parts = self.connection_string.split("://", 1)
                    if len(parts) > 1:
                        main_part = parts[1].split("?", 1)[0]
                        subparts = main_part.split("/")
                        if len(subparts) > 1:
                            db_candidate = subparts[-1]
                            if db_candidate:
                                self.db_name = db_candidate
                except Exception as fallback_err:
                    logging.warning(f"Fallback database name parser failed: {fallback_err}")
                logging.info(f"Using fallback database name parser because parse_uri failed: resolved to '{self.db_name}'")
                
        env_db_name = os.environ.get("MONGODB_DB_NAME")
        if env_db_name:
